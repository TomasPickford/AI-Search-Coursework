...
# IMPLEMENTATION-SEPCIFIC PARAMETERS
repeats = 20
...
def greedySearch():
    ...
    for path in range(num_cities - 3): # stop 2 earlier than normal
        routes = []
        routes_costs = []
        step1_options = unvisited_cities[:] # creates a copy
        for step1 in step1_options:
            step2_options = step1_options[:]
            step2_options.remove(step1)
            for step2 in step2_options:
                if time.time() > start_time + time_limit:
                    return [], float("inf")
                '''
                step3_options = step2_options[:]
                step3_options.remove(step2)
                for step3 in step3_options:
                '''
                    
                routes.append([step1, step2])
                routes_costs.append(dist_matrix[current_city][step1]+dist_matrix[step1][step2])
        
        index = routes_costs.index(min(routes_costs))
        nearest_city = routes[index][0]
        nearest_distance = dist_matrix[current_city][nearest_city]
    ...
    # follow the normal 1-step greedy process for the last 2 cities
    for i in range(2):
    ...
    return partial_tour, partial_tour_length

best_tour = []
best_tour_length = float("inf")
for i in range(repeats):
    new_tour, new_tour_length = greedySearch()
    if time.time() > start_time + time_limit:
        break
