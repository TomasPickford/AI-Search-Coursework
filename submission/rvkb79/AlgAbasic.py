...
phero_decay = 0.2 # rho (0.5)
phero_weight = 1 # alpha (1)
heuristic_weight = 4 # beta (2-5)
w = min(10,num_ants) # this is included to ensure parameters are the same between basic and enhanced implementations
...
def initPheroMatrix():
    ...
    if average_length > 0:
        init_phero_level = (0.5 * w * (w - 1)) / (phero_decay * average_length) # must be the same as the enhanced algorithm
    else:
...
for iteration in range(num_iterations):
    ...
    # find the best tour from this iteration
    ...
    if zero_distance == False: # if all the distances between cities are 0, avoid dividing by the tour length (0) here
        # for each ant, deposit some pheromone on the edges of its tour
        for ant in range(num_ants):
            phero_deposit = 1 / new_tours_lengths[ant]
            for step in range(num_cities - 1):
                phero_matrix[new_tours[ant][step]][new_tours[ant][step+1]] += phero_deposit
            phero_matrix[new_tours[ant][step+1]][new_tours[ant][0]] += phero_deposit # manually update the edge from the last city back to the first
